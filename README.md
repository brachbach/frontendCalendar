Create a calendar web-app.

Required features: This is an open ended problem. You may choose to implement features such as day/month/year views, the ability to drag a calendar event around, and any other features you might see in a calendar application. However, at the very least, I should be able to:

Create an event
Edit an event
Provide a title and description for an event

Implementation: Please implement this project as a single page application (you may choose any framework you’d like).  Bonus points if you build a backend service as well, but it isn’t necessary.

Design: It doesn’t need to look super pretty, but we want a simple, clean and elegant interface. Bonus points for mobile responsiveness.
...
Key things:
-some way to store events (maybe in some time-related fashion to facillitate display by time block)
-some way to create events (could be by a very simple text interface)
-some way to display events (ideally in a calendar-like view, but could just show a list of events). And to select them for editing (see spec)

Store:
-maybe store by numerical form of the date in an object, which then has an array of events for that day?
--YYYYMMDD
-Could theoretically just store all events in one big array, but that seems kinda painful
-definitely gonna need some API to determine what valid dates are, so it's fine if that's a requirement of the store method I want to use

Create:
-to start out, should probably just be a pane that hangs around
-will absolutely need some input validation around key params (esp. date and time)

Display:
-I think it should just be a list to start out with, but moving to a calendar-like view should be a top priority

Components:
-App
-EventEditor (for creation and editing, starts off just for creation)
